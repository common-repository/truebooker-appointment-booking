...
if (isset($_POST['truebooker_user_lastname'])) {
    $truebooker_user_lastname = $_POST['truebooker_user_lastname'];
}
...
if (isset($_POST['truebooker_user_note'])) {
    $truebooker_user_note = $_POST['truebooker_user_note'];
}
...
$result  = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_truebooker_setting ORDER BY truebooker_setting_id DESC LIMIT 18"), ARRAY_A);
	...
	$truebooker_user_service_p = sanitize_text_field($_POST['truebooker_user_service_p']);
	$truebooker_user_dt = sanitize_text_field($_POST['truebooker_user_dt']);
	...
	$data = array(
		...
		global $wpdb, $table_truebooker_setting, $result, $paysandbox, $to, $from, $subject, $details, $details2, $headers, $headers2, $suc_msg;
...
'.esc_html__( 'Thank you,', 'truebooker' ).'';
		...
		if(!empty($result['5']['truebooker_setting_value']) ){
			...
			$suc_msg =esc_html_e('Appointment has been booked successfully!!');
		}
		 $message ['success'] = '<div class="truebooker_success tba-popconfirm"><span>'.$suc_msg.'</span></div>'; 
			...
			if(!empty($result['4']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('There is some error!!');
		    }
			$message ['php_error']= '<div class="truebooker_error tba-popconfirm"><span>'.$er_msg.'</span></div>';
			...
			if(!empty($result['11']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please enter your name');
		    }
			...
			if(!empty($result['10']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please enter your email');
		    }
			if(empty($truebooker_user_email)) {
				...
				if (!filter_var($truebooker_user_email, FILTER_VALIDATE_EMAIL)) {
					$error_msg['truebooker_user_email'] = esc_html_e("Please enter valid email address");
				}
			}
			...
			if(!empty($result['9']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please enter your phone');
		    }
			if(empty($truebooker_user_phone)) {
				...
				if (!preg_match("/^[0-9]*$/", $truebooker_user_phone)) {
					$error_msg['truebooker_user_phone'] = esc_html_e("Only numeric value is allowed");
				}
			}
			...
			if(!empty($result['8']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please select date');
		    }
			...
			if(!empty($result['7']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please select time');
		    }
			...
			if(!empty($result['6']['truebooker_setting_value']) ){
				...
				$er_msg =esc_html_e('Please select service');
		    }
