...
if (isset($_POST['tba_service_id'])) {
    $tba_service_id = sanitize_text_field($_POST['tba_service_id']);
}
	
$table_truebooker_services = $wpdb->prefix . 'truebooker_service';

$truebooker_sql = $wpdb->prepare("SELECT truebooker_service_price FROM $table_truebooker_services WHERE truebooker_service_id =%d", array($tba_service_id));
...
if ($results) {
  ...
  foreach ($results as $result) {
    echo json_encode( $result->truebooker_service_price);
  }
}
